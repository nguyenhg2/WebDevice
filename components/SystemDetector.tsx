...
const gpuAliases: Array<{ pattern: RegExp; keywords: string[] }> = [
  ...
  { pattern: /rx\s*7800/i, keywords: ["7800"] },
  { pattern: /rx\s*7700/i, keywords: ["7700"] },
  ...
  function detect() {
    ...
    try {
      ...
      const notes = [
        ...
        `CPU ước lượng theo ${cores} luồng xử lý trình duyệt báo.`,
...
function chooseGpu(gpus: Gpu[], renderer: string) {
  ...
  if (/intel/i.test(renderer)) return gpus.find((gpu) => gpu.slug.includes("iris-xe")) ?? gpus[0];
  if (/amd|radeon/i.test(renderer)) return gpus.find((gpu) => gpu.brand === "AMD" && gpu.category === "mid") ?? gpus[0];
  if (/nvidia|geforce/i.test(renderer)) return gpus.find((gpu) => gpu.brand === "NVIDIA" && gpu.category === "mid") ?? gpus[0];
...
function chooseCpu(cpus: Cpu[], cores: number) {
  const targetScore = cores >= 16 ? 30000 : cores >= 12 ? 22000 : cores >= 8 ? 15000 : cores >= 4 ? 8500 : 4000;
...
function chooseResolution(gpu: Gpu, ramGb: number): Resolution {
  if (gpu.benchmarkScore >= 22000 && ramGb >= 16) return "1440p";
  if (gpu.benchmarkScore < 2500 || ramGb <= 4) return "720p";
  return "1080p";
}
