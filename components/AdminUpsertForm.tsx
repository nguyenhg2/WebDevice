...
const modules: Record<string, ModuleConfig> = {
  game: {
    label: "Game",
    description: "Quản lý tên game, ảnh, giá, thể loại và cấu hình yêu cầu.",
    keyField: "slug",
    titleField: "name",
    fields: [
      { name: "name", label: "Tên game", required: true, defaultValue: "Game mẫu" },
      { name: "slug", label: "Slug", required: true, defaultValue: "game-mau" },
      { name: "steamId", label: "Steam ID", placeholder: "Có thể để trống" },
      ...
      { name: "coverImage", label: "Ảnh bìa", required: true, defaultValue: "https://example.com/game.jpg" },
    ...
    transform: (v) => ({
      ...
      minSpecs: {
        cpuName: text(v.minCpu),
        gpuName: text(v.minGpu),
        cpuBenchmark: number(v.minCpuBenchmark),
        gpuBenchmark: number(v.minGpuBenchmark),
        ramGb: number(v.minRamGb),
        storageGb: number(v.minStorageGb),
      },
      recSpecs: {
        cpuName: text(v.recCpu),
        gpuName: text(v.recGpu),
        cpuBenchmark: number(v.recCpuBenchmark),
        gpuBenchmark: number(v.recGpuBenchmark),
        ramGb: number(v.recRamGb),
        storageGb: number(v.recStorageGb),
      },
    }),
  },
  gpu: {
    label: "GPU",
    description: "Quản lý card đồ họa, VRAM, điểm hiệu năng và nhóm desktop/laptop.",
    keyField: "slug",
    titleField: "name",
    fields: [
      ...
      { name: "benchmarkScore", label: "Điểm hiệu năng", type: "number", defaultValue: 19600 },
      ...
      { name: "priceRangeVnd", label: "Tầm giá", defaultValue: "7-15trieu" },
      { name: "isLaptop", label: "GPU laptop", type: "checkbox" },
      { name: "commonInVietnam", label: "Phổ biến tại Việt Nam", type: "checkbox", defaultValue: true },
    ],
    transform: (v) => ({
      name: text(v.name),
      slug: text(v.slug),
      brand: text(v.brand),
      benchmarkScore: number(v.benchmarkScore),
      category: text(v.category),
      tdp: nullableNumber(v.tdp),
      vram: nullableNumber(v.vram),
      priceRangeVnd: text(v.priceRangeVnd),
      isLaptop: Boolean(v.isLaptop),
      commonInVietnam: Boolean(v.commonInVietnam),
    }),
  },
  cpu: {
    label: "CPU",
    description: "Quản lý bộ xử lý, số nhân/luồng, socket và điểm hiệu năng.",
    keyField: "slug",
    titleField: "name",
    fields: [
      ...
      { name: "benchmarkScore", label: "Điểm hiệu năng", type: "number", defaultValue: 19500 },
    ...
    transform: (v) => ({
      ...
      socket: nullable(v.socket),
      integratedGpu: nullable(v.integratedGpu),
      priceRangeVnd: text(v.priceRangeVnd),
      commonInVietnam: Boolean(v.commonInVietnam),
    }),
  },
  device: {
    ...
    fields: [
      ...
      { name: "imageUrl", label: "Ảnh sản phẩm", defaultValue: "https://example.com/laptop.jpg" },
      { name: "shopeeUrl", label: "Link Shopee" },
      { name: "tikiUrl", label: "Link Tiki" },
      { name: "phongvuUrl", label: "Link Phong Vũ" },
      { name: "gearvnUrl", label: "Link GearVN" },
    ],
    transform: (v) => ({
      name: text(v.name),
      slug: text(v.slug),
      ...
      storageType: text(v.storageType),
      screenSize: nullableNumber(v.screenSize),
      screenResolution: nullable(v.screenResolution),
      priceVnd: number(v.priceVnd),
      priceRange: text(v.priceRange),
      shopeeUrl: nullable(v.shopeeUrl),
      tikiUrl: nullable(v.tikiUrl),
      phongvuUrl: nullable(v.phongvuUrl),
      gearvnUrl: nullable(v.gearvnUrl),
      imageUrl: nullable(v.imageUrl),
    }),
  },
  benchmark: {
    label: "Benchmark",
    description: "Quản lý FPS cho một cặp game/GPU. Chỉ nhập videoTestUrl nếu là link video đã duyệt.",
    keyField: "id",
    titleField: "gameSlug",
    fields: [
      { name: "gameSlug", label: "Game slug", required: true, defaultValue: "game-mau" },
      ...
      { name: "videoTestUrl", label: "Video test đã duyệt" },
      { name: "confidence", label: "Độ tin cậy", type: "select", options: ["measured", "matched", "manual", "estimated"], defaultValue: "manual" },
      { name: "source", label: "Ghi chú dữ liệu", defaultValue: "Admin nhập thủ công" },
    ],
    transform: (v) => {
      const recommendedSetting = text(v.recommendedSetting);
      const fpsLow = number(v.fpsLow);
      const fpsMedium = number(v.fpsMedium);
      const fpsHigh = number(v.fpsHigh);
      const fpsUltra = number(v.fpsUltra);
      return {
        gameSlug: text(v.gameSlug),
        gpuSlug: text(v.gpuSlug),
        resolution: text(v.resolution),
        fpsLow,
        fpsMedium,
        fpsHigh,
        fpsUltra,
        ...
        avgFps: fpsUltra || fpsHigh || fpsMedium || fpsLow,
        onePercentLow: fpsLow,
        status: text(v.status),
        videoTestUrl: nullable(v.videoTestUrl),
        source: text(v.source),
        confidence: text(v.confidence) || "manual",
  ...
  blogPost: {
    label: "Bài viết",
    description: "Quản lý bài SEO/hướng dẫn. Nội dung hỗ trợ HTML.",
    keyField: "slug",
    titleField: "title",
    fields: [
      ...
      { name: "tags", label: "Thẻ", type: "tags", defaultValue: "game, cau-hinh" },
    ...
    transform: (v) => ({
      ...
      category: text(v.category),
      tags: tags(v.tags),
      metaTitle: text(v.metaTitle),
      metaDescription: text(v.metaDescription),
...
export default function AdminUpsertForm() {
  ...
  async function loadItems(collection = moduleKey, query = search) {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (query.trim()) setStatus(result.items.length ? `Tìm thấy ${result.items.length} bản ghi.` : "Không tìm thấy bản ghi phù hợp.");
      } else {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const result = await response.json();
      if (result.success) {
        setStatus(`Đã lưu ${result.count} bản ghi.`);
  ...
  async function deleteItem(item: Values) {
    ...
    try {
      const response = await fetch("/api/admin/items", {
        ...
        body: JSON.stringify({ collection: moduleKey, key }),
      });
      const result = await response.json();
      if (result.success) {
      ...
      setStatus(`Lỗi xóa: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  return (
    <div className="grid gap-4 xl:grid-cols-[220px_320px_minmax(0,1fr)]">
      <aside className="surface h-fit p-2">
        {moduleEntries.map(([key, item]) => (
          <button
            ...
            className={"block w-full rounded-md px-3 py-2 text-left text-sm font-bold " + (moduleKey === key ? "bg-teal-700 text-white" : "hover:bg-slate-100 dark:hover:bg-gray-800")}
          >
            {item.label}
          </button>
        ))}
      ...
      <section className="surface h-fit p-4">
        <h3 className="text-lg font-black">Tìm bản ghi</h3>
        ...
        <p className="mt-2 text-xs text-slate-500">
          {searchLoading ? "Đang tải..." : search.trim() ? "Đang lọc theo từ khóa." : `Hiển thị ${items.length} bản ghi mới nhất.`}
        </p>
        <button type="button" className="btn mt-3 w-full" onClick={createNew}>
          Tạo mới
        </button>
        <div className="mt-4 max-h-[620px] overflow-auto divide-y divide-slate-100 dark:divide-gray-800">
          ...
          {items.map((item) => {
            ...
            return (
              <article key={key} className="py-3">
                <button type="button" className="block w-full text-left font-bold hover:text-teal-700 dark:hover:text-teal-300" onClick={() => selectItem(item)}>
                  {title}
                </button>
                <p className="mt-1 truncate text-xs text-slate-500">{key}</p>
                <div className="mt-2 flex gap-2">
                  <button type="button" className="rounded-md border px-2 py-1 text-xs font-bold" onClick={() => selectItem(item)}>
                    Sửa
                  </button>
                  <button type="button" className="rounded-md border border-rose-200 px-2 py-1 text-xs font-bold text-rose-600" onClick={() => deleteItem(item)}>
                    Xóa
                  </button>
                </div>
              </article>
      ...
      <form onSubmit={submit} className="surface p-4">
        <div className="border-b border-slate-200 pb-4 dark:border-gray-700">
          <h3 className="text-xl font-black">
            {editingKey ? "Sửa" : "Thêm"} {module.label.toLowerCase()}
          </h3>
        ...
        <div className="mt-5 flex flex-wrap items-center gap-3">
          ...
          <button type="button" className="btn-secondary" onClick={createNew}>
            Hủy sửa
          </button>
...
function renderField(field: Field, value: Values[string], update: (name: string, value: string | boolean) => void) {
  if (field.type === "textarea") {
    return <textarea className="input min-h-28" name={field.name} value={valueForInput(value)} onChange={(event) => update(field.name, event.target.value)} required={field.required} placeholder={field.placeholder} />;
  }
  if (field.type === "checkbox") {
    return <input className="h-5 w-5" name={field.name} type="checkbox" checked={Boolean(value)} onChange={(event) => update(field.name, event.target.checked)} />;
  }
  if (field.type === "select") {
    return (
      <select className="input" name={field.name} value={valueForInput(value)} onChange={(event) => update(field.name, event.target.value)} required={field.required}>
  ...
  return (
    <input
      ...
      placeholder={field.placeholder ?? (field.type === "tags" ? "Nhập cách nhau bằng dấu phẩy" : undefined)}
    />
  );
}
