...
type ModuleConfig = {
  ...
  description: string;
  fields: Field[];
  transform: (values: Record<string, FormDataEntryValue | boolean>) => unknown;
};

const modules: Record<string, ModuleConfig> = {
  game: {
    label: "Trò chơi",
    description: "Thêm hoặc cập nhật trò chơi, ảnh bìa, giá, thể loại và cấu hình yêu cầu.",
    fields: [
  ...
  gpu: {
    label: "Card đồ họa",
    description: "Cập nhật GPU, điểm benchmark, VRAM và nhóm hiệu năng.",
    fields: [
  ...
  benchmark: {
    label: "Đo hiệu năng",
    description: "Cập nhật FPS thực tế hoặc ước tính cho một cặp trò chơi/GPU.",
    fields: [
      ...
      { name: "fpsLow", label: "FPS mức thấp", type: "number", defaultValue: 70 },
      { name: "fpsMedium", label: "FPS mức trung bình", type: "number", defaultValue: 100 },
      { name: "fpsHigh", label: "FPS mức cao", type: "number", defaultValue: 85 },
      { name: "fpsUltra", label: "FPS mức rất cao", type: "number", defaultValue: 60 },
      { name: "recommendedSetting", label: "Thiết lập đề xuất", type: "select", options: ["Thấp", "Trung bình", "Cao", "Rất cao"], defaultValue: "Cao" },
...
const extraModules: Record<string, ModuleConfig> = {
  cpu: {
    label: "Bộ xử lý",
