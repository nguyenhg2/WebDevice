      ...
      <table className="w-full min-w-[820px] table-fixed border-collapse text-sm">
        ...
        <thead className="bg-slate-50 dark:bg-gray-800">
          <tr className="border-b border-slate-200 dark:border-gray-700">
            <SortableTh label="GPU" active={sortKey === "gpu"} direction={direction} onClick={() => sortBy("gpu")} align="left" />
            <SortableTh label="Độ phân giải" active={sortKey === "resolution"} direction={direction} onClick={() => sortBy("resolution")} align="left" />
            <SortableTh label="Low" active={sortKey === "fpsLow"} direction={direction} onClick={() => sortBy("fpsLow")} align="right" />
            <SortableTh label="Medium" active={sortKey === "fpsMedium"} direction={direction} onClick={() => sortBy("fpsMedium")} align="right" />
            <SortableTh label="High" active={sortKey === "fpsHigh"} direction={direction} onClick={() => sortBy("fpsHigh")} align="right" />
            <SortableTh label="Ultra" active={sortKey === "fpsUltra"} direction={direction} onClick={() => sortBy("fpsUltra")} align="right" />
            <SortableTh label="Setting" active={sortKey === "recommendedSetting"} direction={direction} onClick={() => sortBy("recommendedSetting")} align="left" />
            <th className="p-3 text-center font-black">Video</th>
          </tr>
        </thead>
        <tbody>
            ...
            return (
              <tr
                ...
                className={
                  ...
                  (highlightGpu === row.gpuSlug ? "bg-blue-50 dark:bg-blue-950" : "bg-white dark:bg-gray-900")
                }
              >
                <td className="p-3 font-semibold">{gpu?.name ?? row.gpuSlug}</td>
                <td className="p-3">{row.resolution}</td>
                <td className="p-3 text-right tabular-nums">{formatFps(row.fpsLow)}</td>
                <td className="p-3 text-right tabular-nums">{formatFps(row.fpsMedium)}</td>
                <td className="p-3 text-right tabular-nums">{formatFps(row.fpsHigh)}</td>
                <td className="p-3 text-right tabular-nums">{formatFps(row.fpsUltra)}</td>
                <td className="p-3">{row.recommendedSetting}</td>
                <td className="p-3 text-center">
                  <a className="font-semibold text-blue-600" href={row.videoTestUrl ?? "#"} target="_blank" rel="nofollow">
                    Xem
                  </a>
                </td>
              </tr>
