...
export function generateStaticParams() {
  return gpus.map((gpu) => ({ gpuSeoSlug: gpu.slug }));
}
...
export async function generateMetadata({ params }: { params: Promise<{ gpuSeoSlug: string }> }) {
  ...
  return {
    title: gpu ? `${gpu.name} chơi được game gì? Top game ch?y t?t 2026` : "GPU chơi được game gì?",
    description: gpu
      ? `Danh sách game chơi được trên ${gpu.name}, FPS ước tính, setting đề xuất và gìi ? nâng cấp.`
      : "Tra cứu GPU chơi được game gì theo dữ liệu cấu hình phổ biến tại Việt Nam.",
  };
}
